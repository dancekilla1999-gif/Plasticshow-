...
/**
 * Sticky category rail for the price list. Highlights the section currently in
...
export function PriceNav() {
  ...
  useEffect(() => {
    const sections = PRICE_GROUPS.map((g) => document.getElementById(g.id)).filter(
        ...
        {PRICE_GROUPS.map((group) => (
          <li key={group.id}>
            <a
              ...
              className={`block whitespace-nowrap rounded-full border px-5 py-2.5 font-mono text-[10px] uppercase tracking-[0.18em] transition-all duration-400 ${
                active === group.id
