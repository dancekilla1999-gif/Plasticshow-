...
Габарит: cobra ≈ 94 × 108 × 22 мм и ≈ 17 см³ (≈ 21 г PLA), roses
≈ 97 × 111 × 24 мм и ≈ 23 см³ (≈ 29 г PLA) на чашку. Жила: 2.4 мм радиус, ≈ 4.4 мм ширина по контакту, 3.5 мм над
...
Apple размеры самой чашки не публикует — в спецификации есть только габарит
гарнитуры. Поэтому контур выведен из двух независимых источников:

1. **Паспортная глубина 83.4 мм.** Это передне-задний размер всей гарнитуры,
   а самая широкая её часть — как раз чашка. Отсюда короткая ось контура.
2. **Замер по фотографии.** Силуэты обеих чашек выделены по яркости,
   развёрнуты по главным осям (на снимке чашки повёрнуты) и приближены
   суперэллипсом методом наименьших квадратов. Вышло: соотношение сторон
   1.19 и 1.18, показатель n = 2.06 и 2.12 при остаточной ошибке 4–5 %.
   На снимке чашки в накладке, а равномерный офсет чуть скругляет контур и
   занижает соотношение, поэтому в модель заложено 1.20 и n = 2.15.

Итог: 83.4 × 100 мм. Выпуклость купола проверить нечем, и ошибка тут
несимметрична: если реальный купол площе — накладка просто встанет на поясок
и в центре будет зазор; если круче — упрётся макушкой и не сядет. Поэтому
взято 10 мм, с запасом в сторону безопасной ошибки.

Всё это оценка, а не скан, поэтому **сначала печатается
`cobra-x_fit-gauge`** — кольцо высотой 8.5 мм с той же внутренней
поверхностью, что у накладки, и с той же глубиной юбки. Десять минут печати
и два грамма пластика.

* садится с лёгким щелчком и не болтается → печатайте накладки как есть;
* мешает амбушюре сесть на магниты → юбка глубже, чем нужно: `--skirt 5`;
...
| Ключ | По умолчанию | Смысл |
|---|---|---|
| `--cup-w`, `--cup-h` | 83.4, 100 | габарит чашки, мм |
| `--cup-n` | 2.15 | «прямоугольность» контура |
| `--dome` | 10.0 | выпуклость внешней поверхности, мм |
